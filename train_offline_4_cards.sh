...
PROPOSER_MODEL_NAME="your_proposer_model_after_sft"

SOLVER_MODEL_NAME="your_solver_model_after_sft"

PROPOSER_DEEPSPEED_CONFIG="./configs/proposer_deepspeed_config.json"
SOLVER_DEEPSPEED_CONFIG="./configs/solver_deepspeed_config.json"
WANDB_RUN_NAME="llm-gan-$(date +%Y%m%d-%H%M%S)_offline"
OUTPUT_DIR_PROPOSER="output/proposer_$(date +%Y%m%d-%H%M%S)_offline"
OUTPUT_DIR_SOLVER="output/solver_$(date +%Y%m%d-%H%M%S)_offline"
MAX_STEPS=200
SAVE_STEPS=40
...
check_dependencies() {
    ...
    if [ ! -f "configs/accelerate_config.yaml" ]; then
        log_warn "configs/accelerate_config.yaml not found. Using default accelerate configuration."
    fi
...
start_vllm_servers() {
    if [ "$USE_VLLM" -eq 1 ]; then
        ...
        CUDA_VISIBLE_DEVICES="0" trl vllm-serve \
            ...
            &> history_record/dual_grpo_proposer_vllm_offline.out &
        PROPOSER_VLLM_PID=$!
        ...
        CUDA_VISIBLE_DEVICES="1" trl vllm-serve \
            ...
            --gpu-memory-utilization "$VLLM_GPU_MEMORY_UTILIZATION" \
            &> history_record/dual_grpo_solver_vllm_offline.out &
        SOLVER_VLLM_PID=$!
...
main() {
    ...
    if [ -f "configs/accelerate_config.yaml" ]; then
        ACCELERATE_CONFIG_ARG="--config_file configs/accelerate_config.yaml"
    fi
    
    export CUBLAS_WORKSPACE_CONFIG=:4096:8

    # Execute training command
    NCCL_DEBUG=INFO TORCH_NCCL_TRACE_BUFFER_SIZE="100" CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES="2,3" accelerate launch $ACCELERATE_CONFIG_ARG \
        train_offline.py \
