...
WANDB_RUN_NAME="PasoDoble-$(date +%Y%m%d-%H%M%S)_offline"
...
check_dependencies() {
    ...
    # Check if train_offline.py exists
    if [ ! -f "train_offline.py" ]; then
        log_error "train_offline.py not found in current directory"
        return 1
    fi
